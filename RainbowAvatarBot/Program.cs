...
internal static class Program
{
	...
	private static async Task BotOnMessage(Message message)
	{
			...
			if (message.Type == MessageType.Text)
			{
				...
				switch (command)
				{
					...
					case "AVATAR":
					{
						...
						{
							avatars = await BotClient.GetUserProfilePhotosAsync(message.ReplyToMessage.From.Id, limit: 1)
								.ConfigureAwait(false);
		...
		} catch (Exception ex)
		{
			...
			try
			{
				await BotClient.SendTextMessageAsync(chatID, Localization.ErrorOccured, replyToMessageId: message.MessageId)
					.ConfigureAwait(false);
			} catch
			{
				// ignored
	...
	private static async Task<(Stream Result, long Length)> DownloadFile(string fileID)
	{
		...
		var responseMessage =
			await HttpClient.GetAsync(file.FilePath, HttpCompletionOption.ResponseHeadersRead).ConfigureAwait(false);
	...
	private static void GenerateImages(Dictionary<string, uint[]> flags)
	{
		foreach (var (name, rgbValues) in flags)
		{
			...
			foreach (var rgbValue in rgbValues)
			{
				...
				image.Mutate(
					img => img.Fill(
						new Argb32((byte) (rgbValue >> 16), (byte) ((rgbValue >> 8) & 0xFF), (byte) (rgbValue & 0xFF)),
	...
	private static async Task Init()
	{
		if (File.Exists("data/config.json"))
		{
			...
			UserSettings = await JsonSerializer.DeserializeAsync<ConcurrentDictionary<long, string>>(configFile)
				.ConfigureAwait(false);
		}
		...
		Flags = JsonConvert
			.DeserializeObject<Dictionary<string, uint[]>>(await File.ReadAllTextAsync("flags.json").ConfigureAwait(false))
	...
	private static async Task ProcessAndSend(string imageID, string imageUniqueID, string overlayName, Message message)
	{
		...
		} else
		{
			...
			resultImage = new InputMedia(
				resultStream, mediaType switch
				{
		...
		try
		{
			resultMessage = isSticker
				? await BotClient.SendStickerAsync(message.Chat.Id, resultImage, replyToMessageId: message.MessageId)
					.ConfigureAwait(false)
				: await BotClient.SendPhotoAsync(message.Chat.Id, resultImage, replyToMessageId: message.MessageId)
					.ConfigureAwait(false);
		} finally
		{
